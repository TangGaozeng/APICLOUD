;! function(win) {"use strict";
	...
	var $$com = {
		...
		numberNoNum : function NumberNoNum(a, b) {
			return b = b || 2, (!a || isNaN(a)) && (a = 0),a = a.replace(/[^\d.]/g, "").replace(/^\./g, "").replace(/\.{2,}/g, ".").replace(".", "$#$").replace(/\./g, "").replace("$#$", ".").replace(new RegExp("^(\\-)*(\\d+)\\.(\\d{0," + b + "}).*$", "gmi"), "$1$2.$3"),a;
		},
		//格式化金钱
		...
		formatNumber : function (value, num) {
			try{value = $$com.isNullOrUndefined(value)?0:value;}catch(e){value = 0;}
			var a, b, c, i;  
		    a = value.toString();  
		    b = a.indexOf(".");  
		    c = a.length;  
	...
	var defaultsOption = {
		...
		openApp : {
			...
			iosUrl : 'http://www.9999.gd/',
			androidPkg : 'android.intent.action.VIEW',
			mimeType : 'text/html',
			uri : 'http://www.9999.gd'
		},
		imageCache : {
			url : '',
			policy : 'default',
	...
	var $$apicloud = {
		...
		// @key：键
		$getStorage : function(key) {
			return $$apicloud.getStorage(key);
		},
		// 移除存储数据
		// @key：键
		...
		// @key：键
		$removeStorage : function(key) {
			return $$apicloud.removeStorage(key);
		},
		// 清除所有存储数据
